...
abstract class API_TORNEIO {
  ...
  Future<({bool sucesso, bool? is_admin})> check_if_admin(
  ...
  Future<({bool sucesso, err_geral? err})> adicionar_competidor(
      String id_torneio, String id_admin, String nome_competidor);

  // Remove competidor do torneio
  // Só está disponivel quando torneio está em estado de preparo (salão)
  // Verificar se requisitante é admin
  Future<({bool sucesso, err_geral? err})> remover_competidor(
      String id_torneio, String id_admin, String nome_competidor);

  // Seleciona modo de torneio/regras do torneio
  // Verificar se requisitante é admin
  Future<({bool sucesso, err_geral? err})> definir_regras(
  ...
  Future<
      ({
        ...
        bool torneio_fim,
        err_geral? err,
        Map<String, dynamic>? proxima_etapa
      })> criar_proxima_etapa(String id_torneio, String id_admin);
  ...
  // Verificar se requisitante é admin
  Future<({bool sucesso, err_geral? err})> concluir_etapa(
